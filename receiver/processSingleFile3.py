...
import pydicom, json, re, logging, logging.handlers, threading, string
import struct
                ...
                for rule in range(len(self.classify_rules)):
                        ...
                        for entry in range(len(self.classify_rules[rule]['rules'])):
                                ...
                                if op == "notexist":
                                        ...
                                        # sometimes v is of type bytes which breaks in 'not in'. In these cases use decode to convert
                                        try:
                                                v = v.decode("utf-8")
                                        except (UnicodeDecodeError, AttributeError):
                                                pass
                                        #print("v is : %s %s" % (type(v), v))
        ...
        def run(self):
                ...
                while True:
                        ...
                        if not response:
                                ...
                                arriveddir = datadir + '/site/.arrived'.encode('utf-8')
                                ...
                                try:
                                        arrivedfile = os.path.join(arriveddir.decode('utf-8'), _split.sub('', ''.join([ aetitlecaller, " ", aetitlecalled, " ", callerip, " ", dataset.StudyInstanceUID, " ", dataset.SeriesInstanceUID ] )))
                                ...
                                if not os.path.exists(patientdir):
                                        ...
                                        os.umask(oldmask)
                                outdir = datadir + '/site/raw'.encode('utf-8')
                                ...
                                fn = os.path.join(outdir.decode('utf-8'), dataset.StudyInstanceUID, dataset.SeriesInstanceUID)
                                if not os.path.exists(fn):
                                        try:
                                                oldmask = os.umask(0)
                                                os.makedirs(fn,0o1777)
                                                os.umask(oldmask)
                                        except OSError:
                                                print("Error: no permissions to create this path %s" % (fn))
                                        if not os.path.exists(fn):
                                                print("Error: creating path %s did not work" % fn)
                                ...
                                if dataset.PatientID:
                                        patdir = os.path.join(patientdir.decode('utf-8'), dataset.PatientID)
                                        if not os.path.exists(patdir):
                                                oldmask = os.umask(0)
                                                os.makedirs(patdir,0o1777)
                                                os.umask(oldmask)
                                        studydir = os.path.join(patdir, ''.join([dataset.StudyDate, "_", dataset.StudyTime]))
                                        if not os.path.islink( studydir ):
                                                os.symlink( os.path.join(outdir.decode('utf-8'), dataset.StudyInstanceUID), studydir )
                                #else:
                                #  continue # don't  do anything because the file exists already
                                # lets store some data in a series specific file
                                fn3 = os.path.join(outdir.decode('utf-8'), dataset.StudyInstanceUID, dataset.SeriesInstanceUID) + ".json"
                                data = {
                                        'IncomingConnection': { 'AETitleCaller': aetitlecaller, 'AETitleCalled': aetitlecalled, 'CallerIP': callerip },
                                        'Modality': []
                                }
                                #try:
                                ...
                                try:
                                        ...
                                        pass
                                try:
                                        data['Modality'] = list(set(data['Modality'].append(dataset.Modality)))
                                except:
                                        pass
                                try:
                                        data['AcquisitionLength'] = str(dataset[0x51, 0x100a].value)
                                ...
                                try:
                                        ...
                                        pass
                                try:
                                        data['InstitutionName'] = dataset.InstitutionName
                                except:
                                        pass
                                try:
                                        data['OperatorsName'] = dataset.OperatorsName
                                except:
                                        pass
                                try:
                                ...
                                try:
                                        ...
                                        data['StudyDescription'] = 'missing'
                                        logging.error('Error: could not read the StudyDescription %s' % (type(ex).__name__))
                                        pass
                                try:
                                        data['StudyID'] = dataset.StudyID
                                except:
                                        pass
                                try:
                                ...
                                oldmask = os.umask(0)
                                tfn = tempfile.mktemp(dir="/data/site/temp")
                                ...
                                try:
                                        ...
                                        try:
                                                shutil.move(tfn,fn3) # this would rename if possible on its own
                                        except:
                                                print("Error on moving json from temp to raw.")
                                                pass
                                os.chmod(fn3, 0o666)
                                os.umask(oldmask)
                rp.close()
...
if __name__ == "__main__":
        ...
        pidfilename = ''.join([ os.path.dirname(os.path.abspath(__file__)), os.path.sep, '.pids', os.path.sep, 'processSingleFile' , projname , '.pid' ])
