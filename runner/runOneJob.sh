...
while IFS= read -r line; do
    ...
    if [ -d "${ror_folder_path}" ]; then
	    ...
	    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] WARNING Skip this job ($type, $project, $job_number, $image, $user, $date), folder \"${folder}/${dirnam}\" already exists" 
	    jobsToDelete[$jobsCounter]="$line"
	    jobsCounter=$((jobsCounter + 1))
	    continue
    else
	    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] INFO New job, no folder ${ror_folder}/${dirnam} yet"
    fi
...
#	echo "Error: unknown type ${type} (we only support type = ror"
 #   fi
    # and run
    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] INFO Trigger job with type: \"$type\", project: \"$project\", job_number: \"$job_number\", image: \"$image\", user: \"$user\", date: \"$date\" in \"${folder}/${dirnam}\"..."
    cd "${ror_folder}/";
    # pwd
    # add --memory and --cpus to this call to limit damage
    echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] INFO Requested job: $ror trigger --keep --cont \"$image\" --envs ${ROR_CONT_OPTIONS} --job \"${job_number}\" --folder \"${dirnam}\""
    ...
    if [ ! -d "${ror_folder_path}/input" ]; then
	echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] Error: no ror folder path ${ror_folder_path}/input found, no select statement might have matched to an image or study"
        $LOG_TO_SQL --status "ERROR" --message "no ror folder path ${ror_folder_path}/input found, no select statement might have matched to an image or study"
    fi
    if [ ! -d "${ror_folder_path}_output" ]; then
	echo "`date +'%Y-%m-%d %H:%M:%S.%06N'`: [runOneJob.sh] Error: no output folder ${ror_folder_path}_output found, no select statement might have matched to an image or study, or processing failed"
        $LOG_TO_SQL --status "ERROR" --message "no output folder ${ror_folder_path}_output found, no select statement might have matched to an image or study, or processing failed"
    fi
